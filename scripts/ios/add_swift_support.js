  ...
  if ((context.hook === 'after_platform_add' && context.cmdLine.includes('platform add')) ||
    ...
    getPlatformVersionsFromFileSystem(context, projectRoot).then(platformVersions => {
      ...
      try {
        ...
        // If the bridging header doesn't exist, we create it with the minimum Cordova/CDV.h import.
      ...
      // Iterate over all targets and their respective build configurations
      const allTargets = xcodeProject.pbxTargetByName();
      Object.keys(allTargets).forEach(targetName => {
        Object.keys(buildConfigs).forEach(configName => {
          if (!COMMENT_KEY.test(configName)) {
            buildConfig = buildConfigs[configName];
            if (buildConfig.buildSettings && buildConfig.buildSettings.PRODUCT_NAME === targetName) {
      ...
      glob('**/*Bridging-Header*.h', { cwd: pluginsPath }, (error, files) => {
        ...
        // if other bridging headers are found, they are imported in the one already configured in the project.
