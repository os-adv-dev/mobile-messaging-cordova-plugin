...
module.exports = function (ctx) {
    ...
    if (!appGroup) {
        // Fallback to package.json for OutSystems MABS / custom configurations
        var packageJsonPath = path.join(ctx.opts.projectRoot, 'package.json');
        if (fs.existsSync(packageJsonPath)) {
            try {
                var packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
                var cordovaPlugins = packageJson.cordova && packageJson.cordova.plugins;
                var pluginVariables = cordovaPlugins && cordovaPlugins['com-infobip-plugins-mobilemessaging'];
                if (pluginVariables && pluginVariables.IOS_EXTENSION_APP_GROUP) {
                    appGroup = pluginVariables.IOS_EXTENSION_APP_GROUP;
                    console.log('Infobip: Loaded IOS_EXTENSION_APP_GROUP from package.json: ' + appGroup);
                }
            } catch (err) {
                console.log('WARNING: Failed to parse package.json: ' + err.message);
            }
        }
    }

    if (!appGroup) {
        console.log('WARNING: IOS_EXTENSION_APP_GROUP variable not found in config.xml or package.json. Skipping NSE integration.');
        return;
    }
